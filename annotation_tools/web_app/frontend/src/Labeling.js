...
const attributes = ['','has_colour','has_size','has_name','has_orientation','has_thickness','has_height','has_length','has_radius','has_slope','has_width','has_base','has_distance','has_block_type']
const dropdown_choices = {
 'dialogue_type': ['HUMAN_GIVE_COMMAND','get_memory','put_memory'],
 'action_type': ['BUILD','COPY','NOOP','SPAWN','RESUME','FILL','DESTROY','MOVE','UNDO','STOP','DIG','FREEBUILD','DANCE'],
 'relative_direction': ['LEFT','RIGHT','DOWN','FRONT','BACK','AWAY','INSIDE','NEAR','OUTSIDE'],
...
function Labeling({actiondict, message, goToEnd}) {
 ...
 function renderRows(dict,col,prevkeys) {
   ...
   Object.keys(dict).forEach(function(key) {
     if (typeof(dict[key]) == 'object'){
       //recurse
       rows = rows.concat(renderRows(dict[key],col+1,prevkeys.concat(key)));
     } else {
       var r = [];
       for (var i=0; i<col-1; i++) {
         if (i === col-2 && prevkeys.length > 0) {
           r.push(<TableCell align="center" className={classes.category} scope="row">{prevkeys[prevkeys.length - 1]}</TableCell>)
         } else {
       ...
       if (key.includes('has_')){
         r.push(<TableCell align="center" scope="row">
                   <FormControl>
                     <Select
                       value={key}
                       onChange={(event) => handleChangeKey(event,prevkeys.slice().concat(key))}
                       input={<Input />}
                       >
         ...
         r.push(<TableCell align="center" component="th" scope="row">{key}</TableCell>)
       }

       //check if need dropdown or text edit
       if (key in dropdown_choices) {
         r.push(<TableCell align="center">
                   <FormControl>
                     <Select
                       value={getVal(state.addict,prevkeys.slice().concat(key))}
                       onChange={(event) => handleChangeValue(event,prevkeys.slice().concat(key))}
                       input={<Input />}
                       inputProps={{
         ...
         r.push(<TableCell align="center">
           <Input
           defaultValue={getVal(state.addict,prevkeys.slice().concat(key))}
           onChange={(event) => handleChangeValue(event,prevkeys.slice().concat(key))}
           inputProps={{
       ...
       if (dict[key].includes(' ')) {
         r = [];
         for (var j=0; j<col; j++) {
           r.push(<TableCell align="center" component="th" scope="row"></TableCell>)
         }
         r.push(<TableCell>
            Do any of the words describe the other? <br></br>
            <FormControl>
               <Select
                onChange={(event) => handleAddKey(event,prevkeys.slice().concat(key))}
                input={<Input />}
                >
                {renderMenuItems(attributes)}
              </Select>
 ...
 return (
   ...
   <Table>
     <TableBody>
       {renderRows(state.addict,0,[])}
     </TableBody>
   </Table>
   </Paper>
   <Button variant="contained" color="primary" onClick={doneClicked}>Done</Button>
   </div>
 );
}
