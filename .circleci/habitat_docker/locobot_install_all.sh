...
install_packages () {
	...
	do
		if [ $(dpkg-query -W -f='${Status}' $package_name 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
		    sudo -E apt -y install $package_name
		else
...
# STEP 0 - Make sure you have installed Ubuntu 18.04, and upgrade to lastest dist
...
if [ $(dpkg-query -W -f='${Status}' ros-melodic-desktop-full 2>/dev/null | grep -c "ok installed") -eq 0 ]; then 
	...
	sudo apt-get update
	sudo apt-get update
	sudo apt search melodic
	sudo apt-get -y install ros-melodic-desktop-full python-rosdep
...
if [ ! -d "$LOCOBOT_FOLDER/src/pyrobot" ]; then
	cd $LOCOBOT_FOLDER/src
	git clone https://github.com/soumith/pyrobot.git
	cd pyrobot
