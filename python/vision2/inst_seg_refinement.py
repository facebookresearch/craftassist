...
class MaskModel(VoxelCNN):
    ...
    def get_non_air_mask(self, x):
        ...
        with torch.no_grad():
            ...
            for i in range(self.n_scales // 2):
                x = nn.MaxPool3d(kernel_size=2)(x)
        return (x != 0).float()

    def forward_and_loss(self, x, mask, target):
        B = x.size()[0]
        ## only predict at non-air blocks
        non_air_mask = self.get_non_air_mask(x).view(B, -1)
        ...
        loss.append(
            nn.BCEWithLogitsLoss()(
                out_mask.view(B, -1)[idx], targets[out_scale + 1].view(B, -1)[idx]
            )
        )
        return loss
    ...
    def predict(self, x, mask):
        ...
        feat_maps = self(x, mask)
        out_mask = (self.mask_layer(feat_maps[self.n_scales // 2 - 1]) > 0).float() * non_air_mask
    ...
    def train_epochs(
        ...
        for m in range(max_epochs):
            ...
            for i, ls in enumerate(zip(*loss)):
                print("Resolution 1/{} loss: {}".format(2 ** i, sum(ls) / len(ls)))
    ...
    def subsample_targets(self, target):
        ...
        for i in range(self.n_scales):
            with torch.no_grad():
                t = nn.MaxPool3d(kernel_size=2)(targets[-1].unsqueeze(1)).squeeze(1)
                targets.append(t)
        return targets

    def sample_an_epoch(self, data, samples_per_house):
        x, mask, target = [], [], []
        for obj, anno, cls in data:
...
def prepare_data(training_data, validation_data):
    ...
    tags = dict()
    for _, inst_anno, inst_cls in training_data:
        for i in range(len(inst_cls)):
            t = inst_cls[i]
            if t not in tags:
                tags[t] = 0
...
if __name__ == "__main__":
    ...
    training_data, validation_data, tags = prepare_data(training_data, validation_data)

    print("loading finished!")

    model = MaskModel(opts=dict(voxel_embedding_dim=16, n_scales=4, gpu_id=0), tags=tags)
    model.train_epochs(
        training_data, validation_data, "./model.pth", max_epochs=500, batch_size=64, lr=1e-3
    )
