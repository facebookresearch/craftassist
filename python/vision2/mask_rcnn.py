...
from inst_seg import compute_bce_f1
...
class ThreedMaskRCNN(VoxelCNN):
    def __init__(self, tags=[], opts={}, model_path=None):
        ...
        # [proposal, mask] weights
        self.cost_weights = opts.get("cost_weights", [10, 1])
        self.mask_prediction_size = opts.get("mask_size", 8)
    ...
    def _generic_feature_cnn(self, N, M, E, C, n_blocks):
        ...
        for i in range(n_blocks):
            layer = nn.Sequential(
                nn.Conv3d(in_channels, channels[i], kernel_size=5, stride=1, padding=2),
    ...
    def _inst_seg_net(self, C, O, N):
        ...
        class Classifier(nn.Module):
            ...
            def forward(self, x):
                ...
                x = nn.AvgPool3d(kernel_size=(H, W, D))(x)
                x = x.view(B, -1)
                return self.net(x)
    ...
    def train_epochs(
        ...
        for m in range(max_epochs):
            ...
            for i, npos in enumerate(zip(*n_positives)):
                avg_pos.append((i + 1, sum(npos) / len(training_data)))
            print("Average positives per batch: {}".format(avg_pos))
...
if __name__ == "__main__":
    ...
    training_data = [(make_center(td[0], size), make_center(td[1], size)) for td in training_data]
    validation_data = [
        (make_center(vd[0], size), make_center(vd[1], size)) for vd in validation_data
    ]
    ...
    model = ThreedMaskRCNN(
        opts=dict(
            ...
            S=4,
            sizes_x=[1, 2, 3, 4],
            ratios_y=[0.33, 0.5, 1, 2, 3],
            ratios_z=[0.33, 0.5, 1, 2, 3],
            mask_size=8,
    ...
    model.train_epochs(
        training_data, validation_data, "./model.pth", batch_size=16, max_epochs=500, lr=1e-3
    )
