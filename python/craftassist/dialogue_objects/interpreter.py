...
class Interpreter(DialogueObject):
    ...
    def handle_build(self, speaker, d) -> Tuple[Optional[str], Any]:
        ...
        if "reference_object" in d:
            ...
            objs = interpret_reference_object(
                self, speaker, d["reference_object"], limit=repeat, ignore_mobs=True
            )
            if len(objs) == 0:
    ...
    def handle_freebuild(self, speaker, d) -> Tuple[Optional[str], Any]:
        if "location" in d:
            # FreeBuild from a location
            location_d = d.get("location", {"location_type": "SPEAKER_LOOK"})
            location = interpret_location(self, speaker, location_d)
        elif "reference_object" in d:
            # free build from a block object
            # TODO (Zhuoyuan): block list is saved as an unordered dict
            #   randomly choose one
            objs = interpret_reference_object(
                self, speaker, d["reference_object"], ignore_mobs=True
            )
            if len(objs) == 0:
                raise ErrorWithResponse("I don't understand what you want me to free-build.")
            obj = objs[0]
            location = random.choice(obj.keys())
        else:
            raise ErrorWithResponse("Specify a position or an object to free-build.")

        task_data = {"location": location, "action_dict": d}
        self.append_new_task(tasks.FreeBuild, task_data)
        self.finished = True
        return None, None
    ...
    def handle_fill(self, speaker, d) -> Tuple[Optional[str], Any]:
        ...
        if holes is None:
            self.agent.send_chat("I don't understand what holes you want me to fill.")
            return None, None
        for hole in holes:
            _, hole_info = hole
        ...
        # FIXME should go through dialogue manager TODO
        if len(holes) > 1:
            self.agent.send_chat("Ok. I'll fill up the holes.")
        else:
            self.agent.send_chat("Ok. I'll fill that hole up.")
        self.finished = True
        return None, None
