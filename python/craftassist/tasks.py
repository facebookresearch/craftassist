...
# TODO don't allow placing of blocks that fall to the ground
# TODO store the blockobject in memory,
# flag as incomplete, flag it when it is complete
class FreeBuild(Task):
    def __init__(self, agent, task_data, featurizer=None):
        # task_data: a dictionary
        #   "location": last block
        # model: Voxel-CNN model
        super(FreeBuild, self).__init__(featurizer=featurizer)

        self.origin = task_data.get("location")
        self.location = self.origin  # current location

        # currently, assume 10 new blocks
        self.remaining = 10
    ...
    def step(self, agent):
        ...
        xyz_pred, pred_block = agent.generator.generator.full_predict_mc(agent, self.location)

        # Create a new building task so we can reuse the codes
        origin = np.array(xyz_pred)
        new_task_data = {"blocks_list": [((0, 0, 0), pred_block)], "origin": origin}
        task = Build(agent, new_task_data)
        agent.memory.task_stack_push(task, parent_memid=self.memid)

        # new_block = tuple(xyz_pred), pred_block
        self.location = xyz_pred

        self.remaining -= 1
        if self.remaining == 0:
            self.finished = True
            if self.verbose:
                agent.send_chat("I finished free-building")
            return
    ...
    def __repr__(self):
        return "<FreeBuild @ >".format(self.origin)
