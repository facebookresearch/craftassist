...
def prepare_unary_and_features(np_blocks, voxel_model, batch_size):
    """
    Given a numpy array of an object (e.g., house) voxel representation,
    compute the unary potential and pairwise features for CRF, from voxel_model.
    """
    ...
    scores = voxel_model.predict_object(np_blocks, batch_size)
    ...
    embed_table = voxel_model.embedding.weight.data.detach().cpu().numpy()
    features = {}
    for l in unary.keys():
        x, y, z = l
        if mbi.voxel_group:
