...
from voxel_cnn_base import VoxelCNN, Flatten
from center_voxel_cube import get_batch_voxel_center_cubes


class LocalVoxelCNN(VoxelCNN):
    cube_size = 9
    """
    A base class for classifying a voxel fully depending on its local 9x9x9 neighborhood
    This is the vision model released for the v0 bot.
    """

    def __init__(self, tags=[], opts={}, model_path=None):
        super(LocalVoxelCNN, self).__init__(opts, tags, model_path)
        ...
        for m in range(max_epochs):
            ...
            for i in range(0, len(training_data), batch_size):
                ...
                if self.opts["data_augmentation"]:
                    ipt = [self._data_augmentation(cube) for cube in ipt]
                ipt = torch.stack(ipt, dim=0)
                out = torch.stack(out, dim=0)
                outhat = self(ipt)
                opt.zero_grad()
                l = loss_fn(self.opts["gpu_id"], outhat.double(), out)
                loss.append(l)
                l.backward()
                opt.step()
            ...
            self.validate(training_data_np, pred_fn, batch_size)
            logging.info("Eval on validation data:")
            self.validate(validation_data, pred_fn, batch_size)

        self.save(save_model_path)

    def validate(self, validation_data, pred_fn, batch_size):
        inputs, out = zip(*validation_data)
        pred = self._predict(inputs, pred_fn, batch_size, False)
        ...
        logging.info("Overall: {}".format(np.mean([x for v in res.values() for x in v])))
        for k in res.keys():
            res[k] = round(np.mean(res[k]), 2)
        res = sorted(list(res.items()), key=lambda x: x[0])
        logging.info("Acc: {}".format([r[1] for r in res]))

    def _predict(self, cubes, pred_fn, batch_size, soft):
        """
    ...
    def predict_object(self, np_blocks, batch_size):
        """
        Given a numpy array of an object, predict the log-prob score
