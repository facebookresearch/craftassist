...
class DigSomeShape(TemplateObject):
    def __init__(self, node):
        ...
        self._child = shape_type(node=node)
...
class DigDimensions(TemplateObject):
    def add_generate_args(self, index=0, templ_index=0):
        self.node.has_length = random.choice(range(2, 15))
        self.node.has_width = random.choice(range(15, 30))
        self.node.has_depth = random.choice(range(30, 45)) if pick_random() else None
